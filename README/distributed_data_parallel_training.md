...
## Why do we use Pytorch Distributed Data Parallel class?
...
We will go throught each flag as the following:
...
- Pass the number of processes that you wish to run by passing `--nproc_per_node=3` in. In the above case we want our neural network be run on 3 GPUs `1,6,3` simultaneously.

- Passing `--master_port=$RANDOM` is not necessary but without it you may not be able to get an open master port. However, passing `--master_port=$RANDOM` makes chosing the master port random so having an open master port is guaranteed.
...
[`main.py`](https://github.com/sdamadi/image-classification/blob/main/main.py) creates an instance of `Distributed` class which is in `utils/distributed.py`. To create that instace we use the following:
...
- Status of whether we are using the distributed training, i.e., `distributed.args.distributed`. When we set `--nproc_per_node=1` this will be `False` since the trainig process is done on one single GPU. Otherwise it is set to `True`.
...
- `args.gpu_ids` is used print out which GPUs are allocated for training the current run.

## How [`utils/distributed.py`](https://github.com/sdamadi/image-classification/blob/main/utils/distributed.py) module works?

This modlue as we explained in the above creates `Distributed` object. This class is initialized as follows:

-  `self.args.distributed` is initialized to be `False`. This is because in a case where `--nproc_per_node=1`, one wants the training be run on a single GPU. Therefore, the code execute the else part of `if self.args.distributed`.
